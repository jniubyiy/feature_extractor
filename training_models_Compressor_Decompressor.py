...
Потери: difference_loss, diff_smooth_loss_parnet.
"""
...
def run_validation(compressor, opt_comp, decompressor, opt_decomp, val_loader, epoch):
    ...
    sum_diff = 0.0; sum_total = 0.0; sum_psnr = 0.0
    sum_span_comp = 0.0; sum_span_rec = 0.0
    n_batches = 0
    with torch.no_grad():
        for parnets, _ in val_loader:
            if COMPRESSOR_DEVICE == DECOMPRESSOR_DEVICE:
                p_c = parnets.to(COMPRESSOR_DEVICE); p_d = p_c
            else:
                p_c = parnets.to(COMPRESSOR_DEVICE); p_d = parnets.to(DECOMPRESSOR_DEVICE)
            comp = compressor(p_c)
            if COMPRESSOR_DEVICE != DECOMPRESSOR_DEVICE:
                comp = comp.to(DECOMPRESSOR_DEVICE)
            rec = decompressor(comp)
            diff = difference_loss(rec, p_d)
            total = PARNET_DIFF_LOSS_WEIGHT * diff
            sum_diff += diff.item(); sum_total += total.item()
            sum_psnr += compute_psnr(rec, p_d)
            sum_span_comp += compute_span(comp)
            sum_span_rec += compute_span(rec)
            n_batches += 1
    avg_diff = sum_diff / n_batches; avg_total = sum_total / n_batches
    avg_psnr = sum_psnr / n_batches
    avg_span_comp = sum_span_comp / n_batches; avg_span_rec = sum_span_rec / n_batches
    ...
    for idx in indices:
        ...
        if COMPRESSOR_DEVICE == DECOMPRESSOR_DEVICE:
            p_c = parnet.unsqueeze(0).to(COMPRESSOR_DEVICE); p_d = p_c
        else:
            p_c = parnet.unsqueeze(0).to(COMPRESSOR_DEVICE); p_d = parnet.unsqueeze(0).to(DECOMPRESSOR_DEVICE)
        with torch.no_grad():
            comp = compressor(p_c)
            if COMPRESSOR_DEVICE != DECOMPRESSOR_DEVICE: comp = comp.to(DECOMPRESSOR_DEVICE)
            rec = decompressor(comp)
            diff_val = difference_loss(rec, p_d).item()
            psnr_val = compute_psnr(rec, p_d)
            span_comp_val = compute_span(comp); span_rec_val = compute_span(rec)
            examples.append({
                'example_id': eid,
                'original_parnet': p_d.squeeze(0).cpu(),
                'reconstructed_parnet': rec.squeeze(0).cpu(),
                'compressed_parnet': comp.squeeze(0).cpu(),
                'diff': diff_val, 'psnr': psnr_val,
                'span_compressed': span_comp_val, 'span_reconstructed': span_rec_val,
            })

    # Временное сохранение чекпоинтов
    temp_paths = []
    for name, model, opt in [(COMPRESSOR_NAME, compressor, opt_comp),
                             (DECOMPRESSOR_NAME, decompressor, opt_decomp)]:
    ...
    d_ckpt = torch.load(DECODER_CHECKPOINT, map_location=DECODER_DEVICE, weights_only=False)
    decoder.load_state_dict(d_ckpt if not isinstance(d_ckpt, dict) or "model_state_dict" not in d_ckpt else d_ckpt["model_state_dict"])
    decoder.eval()
    ...
    del decoder, examples; gc.collect()
    if torch.cuda.is_available(): torch.cuda.empty_cache()
    ...
    for (name, model, opt), path in zip([(COMPRESSOR_NAME, compressor, opt_comp), (DECOMPRESSOR_NAME, decompressor, opt_decomp)], temp_paths):
        ckpt = torch.load(path, map_location='cpu', weights_only=False)
        model.load_state_dict(ckpt['model_state_dict'])
        opt.load_state_dict(ckpt['optimizer_state_dict'])
        os.remove(path)

    compressor = torch.compile(compressor, backend="aot_eager")
    decompressor = torch.compile(decompressor, backend="aot_eager")
...
def collect_test_data(compressor, decompressor, dataset):
    ...
    with torch.no_grad():
        for idx in indices:
            ...
            fname = os.path.basename(dataset.files[idx]); eid = os.path.splitext(fname)[0]
            if COMPRESSOR_DEVICE == DECOMPRESSOR_DEVICE:
                p_c = parnet.unsqueeze(0).to(COMPRESSOR_DEVICE); p_d = p_c
            else:
                p_c = parnet.unsqueeze(0).to(COMPRESSOR_DEVICE); p_d = parnet.unsqueeze(0).to(DECOMPRESSOR_DEVICE)
...
def save_example_images(base_dir, item, decoder):
    ...
    orig = item['original_parnet']; rec = item['reconstructed_parnet']; comp = item['compressed_parnet']
    parnet_to_pil(orig).save(os.path.join(base_dir, "original_parnet.png"))
    parnet_to_pil(rec).save(os.path.join(base_dir, "reconstructed_parnet.png"))
    parnet_to_pil((rec - orig).abs()).save(os.path.join(base_dir, "difference_parnet.png"))
    decoder.eval()
    with torch.no_grad():
        orig_img = decoder(orig.unsqueeze(0).to(DECODER_DEVICE)).squeeze(0).cpu()
        rec_img = decoder(rec.unsqueeze(0).to(DECODER_DEVICE)).squeeze(0).cpu()
    ...
    parnet_to_pil((rec_img - orig_img).abs()).save(os.path.join(base_dir, "difference_decoded.png"))
    with open(os.path.join(base_dir, "metrics.txt"), 'w') as f:
        f.write(f"Diff (hypo): {item['diff']:.6f}\nPSNR: {item['psnr']:.2f} dB\n")
        f.write(f"Span compressed: {item['span_compressed']:.4f}\nSpan reconstructed: {item['span_reconstructed']:.4f}\n")
    for name, tensor in [("original_parnet", orig), ("reconstructed_parnet", rec), ("compressed_parnet", comp)]:
...
def run_tests(compressor, opt_comp, decompressor, opt_decomp, dataset, epoch):
    ...
    temp_paths = []
    for name, model, opt in [(COMPRESSOR_NAME, compressor, opt_comp), (DECOMPRESSOR_NAME, decompressor, opt_decomp)]:
        ...
        temp_paths.append(path)
    del compressor, opt_comp, decompressor, opt_decomp; gc.collect()
    if torch.cuda.is_available(): torch.cuda.empty_cache()
    decoder = Decoder(**DECODER_CONFIG).to(DECODER_DEVICE)
    d_ckpt = torch.load(DECODER_CHECKPOINT, map_location=DECODER_DEVICE, weights_only=False)
    decoder.load_state_dict(d_ckpt if not isinstance(d_ckpt, dict) or "model_state_dict" not in d_ckpt else d_ckpt["model_state_dict"])
...
def train_epoch(compressor, decompressor, train_loader, opt_comp, opt_decomp):
    ...
    for batch_idx, (parnets, indices) in enumerate(train_loader):
        ...
        # Фаза 1: декомпрессор
        for p in compressor.parameters(): p.requires_grad = False
        for p in decompressor.parameters(): p.requires_grad = True
        ...
        with torch.no_grad():
            ...
            if COMPRESSOR_DEVICE != DECOMPRESSOR_DEVICE: comp = comp.to(DECOMPRESSOR_DEVICE)
        rec = decompressor(comp)
        loss_1 = (PARNET_DIFF_LOSS_WEIGHT * difference_loss(rec, p_d) +
                  DIFF_SMOOTH_LOSS_WEIGHT * diff_smooth_loss_parnet(rec, p_d))
        loss_1.backward()
        for name, param in decompressor.named_parameters():
            if param.grad is not None: saved_grads_decomp[name] = param.grad.clone().cpu()
        opt_decomp.zero_grad()
        loss1_val = loss_1.item()
        diff_smooth_val_1 = (DIFF_SMOOTH_LOSS_WEIGHT * diff_smooth_loss_parnet(rec, p_d)).item()
        del comp, rec, loss_1

        # Фаза 2: компрессор
        for p in compressor.parameters(): p.requires_grad = True
        for p in decompressor.parameters(): p.requires_grad = False
        opt_comp.zero_grad()
        comp = compressor(p_c)
        if COMPRESSOR_DEVICE != DECOMPRESSOR_DEVICE: comp = comp.to(DECOMPRESSOR_DEVICE)
        rec = decompressor(comp)
        loss_2 = (PARNET_DIFF_LOSS_WEIGHT * difference_loss(rec, p_d) +
                  DIFF_SMOOTH_LOSS_WEIGHT * diff_smooth_loss_parnet(rec, p_d))
        loss_2.backward()
        for name, param in compressor.named_parameters():
            if param.grad is not None: saved_grads_comp[name] = param.grad.clone().cpu()
        opt_comp.zero_grad()
        loss2_val = loss_2.item()
        diff_smooth_val_2 = (DIFF_SMOOTH_LOSS_WEIGHT * diff_smooth_loss_parnet(rec, p_d)).item()
        del comp, rec, loss_2
        ...
        for name, param in decompressor.named_parameters():
            if name in saved_grads_decomp:
                ...
                param.grad = None
        opt_decomp.step(); opt_decomp.zero_grad(); saved_grads_decomp.clear()
        for name, param in compressor.named_parameters():
            if name in saved_grads_comp:
                param.grad = saved_grads_comp[name].to(param.device)
            else:
                param.grad = None
        opt_comp.step(); opt_comp.zero_grad(); saved_grads_comp.clear()

        total_loss1 += loss1_val; total_loss2 += loss2_val
        total_diff_smooth_1 += diff_smooth_val_1; total_diff_smooth_2 += diff_smooth_val_2

        span_comp_batch = compute_span(compressor(parnets.to(COMPRESSOR_DEVICE)))
        span_rec_batch = compute_span(torch.zeros(1))
...
def train():
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        print(f"\n--- Epoch {epoch} ---")
        avg_loss1, avg_loss2, avg_ds1, avg_ds2 = train_epoch(compressor, decompressor, train_loader, opt_comp, opt_decomp)
        print(f"Epoch {epoch:3d} | Phase1 (Decomp): {avg_loss1:.6f} | Phase2 (Comp): {avg_loss2:.6f}")
        ...
        if epoch % TEST_EVERY_EPOCHS == 0:
            ...
            compressor, opt_comp, decompressor, opt_decomp = run_tests(compressor, opt_comp, decompressor, opt_decomp, train_dataset, epoch)
