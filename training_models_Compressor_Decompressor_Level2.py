# training_models_Compressor_Decompressor_Level2.py
...
from model_ParnetCompressor import ParnetDecompressor   # первого уровня
from model_Autoencoder import Decoder
from config_training_models_Compressor_Decompressor_Level2 import *
...
COMPRESSOR_NAME = "compressor_level2"
DECOMPRESSOR_NAME = "decompressor_level2"

# ---------------------- Датасет ----------------------
class CompressedParnetDataset(Dataset):
    """Загружает сжатые парнеты первого уровня [4, H/2, W/2]."""
    ...
    def __getitem__(self, idx):
        ...
        return data['compressed_parnet']  # [4, H/2, W/2]
...
# ---------------------- Валидация (последовательная загрузка) ----------------------
def run_validation(compressor, opt_comp, decompressor, opt_decomp, val_loader, epoch):
    ...
    # Сохраняем несколько примеров с полной визуализацией
    val_dataset = val_loader.dataset
    indices = random.sample(range(len(val_dataset)), min(NUM_TEST_EXAMPLES, len(val_dataset)))

    examples = []
    for idx in indices:
        parnet_4ch = val_dataset[idx]   # [4, H/2, W/2]
        eid = int(os.path.splitext(os.path.basename(val_dataset.files[idx]))[0])
        if COMPRESSOR_DEVICE == DECOMPRESSOR_DEVICE:
            parnet_comp = parnet_4ch.unsqueeze(0).to(COMPRESSOR_DEVICE)
            parnet_decomp = parnet_comp
        else:
            parnet_comp = parnet_4ch.unsqueeze(0).to(COMPRESSOR_DEVICE)
            parnet_decomp = parnet_4ch.unsqueeze(0).to(DECOMPRESSOR_DEVICE)
        with torch.no_grad():
            compressed = compressor(parnet_comp)
            if COMPRESSOR_DEVICE != DECOMPRESSOR_DEVICE:
                compressed = compressed.to(DECOMPRESSOR_DEVICE)
            rec_4ch = decompressor(compressed)   # [1, 4, H/2, W/2]
            diff_val = difference_loss(rec_4ch, parnet_decomp).item()
            psnr_val = compute_psnr(rec_4ch, parnet_decomp)
        examples.append({
            'example_id': eid,
            'original_4ch': parnet_decomp.squeeze(0).cpu(),
            'reconstructed_4ch': rec_4ch.squeeze(0).cpu(),
            'diff': diff_val,
            'psnr': psnr_val,
        })
    ...
    # Загружаем ParnetDecompressor первого уровня
    print("Loading Level1 Decompressor for validation...")
    decomp1 = ParnetDecompressor(**LEVEL1_DECOMPRESSOR_CONFIG).to(DECOMPRESSOR_DEVICE)
    ckpt1 = torch.load(LEVEL1_DECOMPRESSOR_CHECKPOINT, map_location=DECOMPRESSOR_DEVICE, weights_only=False)
    decomp1.load_state_dict(ckpt1)
    decomp1.eval()

    # Преобразуем 4-канальные парнеты в 3-канальные полного размера
    for item in examples:
        orig_4ch = item['original_4ch'].unsqueeze(0).to(DECOMPRESSOR_DEVICE)
        rec_4ch = item['reconstructed_4ch'].unsqueeze(0).to(DECOMPRESSOR_DEVICE)
        with torch.no_grad():
            item['original_3ch'] = decomp1(orig_4ch).squeeze(0).cpu()
            item['reconstructed_3ch'] = decomp1(rec_4ch).squeeze(0).cpu()
    ...
    # Загружаем Decoder
    print("Loading Decoder for validation...")
    decoder = Decoder(**DECODER_CONFIG).to(DECODER_DEVICE)
    dec_ckpt = torch.load(DECODER_CHECKPOINT, map_location=DECODER_DEVICE, weights_only=False)
    decoder.load_state_dict(dec_ckpt)
    decoder.eval()

    # Декодируем в изображения и сохраняем
    for item in examples:
        ...
        # Парнеты (4-канальные) сохраняем как есть? Можем сохранить только 3-канальные и изображения.
        # Сохраним 3-канальные парнеты и декодированные изображения.
        orig_3ch = item['original_3ch']
        rec_3ch = item['reconstructed_3ch']
        parnet_to_pil(orig_3ch).save(os.path.join(base_dir, "original_parnet_3ch.png"))
        parnet_to_pil(rec_3ch).save(os.path.join(base_dir, "reconstructed_parnet_3ch.png"))
        diff_3ch = (rec_3ch - orig_3ch).abs()
        parnet_to_pil(diff_3ch).save(os.path.join(base_dir, "difference_parnet_3ch.png"))

        with torch.no_grad():
            orig_img = decoder(orig_3ch.unsqueeze(0).to(DECODER_DEVICE)).squeeze(0).cpu()
            rec_img = decoder(rec_3ch.unsqueeze(0).to(DECODER_DEVICE)).squeeze(0).cpu()
        ...
        with open(os.path.join(base_dir, "metrics.txt"), 'w') as f:
            f.write(f"Diff (4ch): {item['diff']:.6f}\nPSNR (4ch): {item['psnr']:.2f} dB\n")
    ...
    compressor = ParnetCompressorLevel2(**COMPRESSOR_CONFIG).to(COMPRESSOR_DEVICE)
    decompressor = ParnetDecompressorLevel2(**DECOMPRESSOR_CONFIG).to(DECOMPRESSOR_DEVICE)
...
def run_tests(compressor, opt_comp, decompressor, opt_decomp, dataset, epoch):
    ...
    # Собираем данные аналогично валидации, но без вычисления средних метрик (только примеры)
    compressor.eval()
    decompressor.eval()
    ...
    with torch.no_grad():
        for idx in indices:
            ...
            if COMPRESSOR_DEVICE != DECOMPRESSOR_DEVICE:
                compressed = compressed.to(DECOMPRESSOR_DEVICE)
            rec_4ch = decompressor(compressed)
            diff_val = difference_loss(rec_4ch, parnet_decomp).item()
            psnr_val = compute_psnr(rec_4ch, parnet_decomp)
            examples.append({
                'example_id': eid,
    ...
    # Загружаем декомпрессор первого уровня
    print("Loading Level1 Decompressor for testing...")
    decomp1 = ParnetDecompressor(**LEVEL1_DECOMPRESSOR_CONFIG).to(DECOMPRESSOR_DEVICE)
    ckpt1 = torch.load(LEVEL1_DECOMPRESSOR_CHECKPOINT, map_location=DECOMPRESSOR_DEVICE, weights_only=False)
    decomp1.load_state_dict(ckpt1)
    decomp1.eval()
    ...
    # Загружаем Decoder
    print("Loading Decoder for testing...")
    decoder = Decoder(**DECODER_CONFIG).to(DECODER_DEVICE)
    dec_ckpt = torch.load(DECODER_CHECKPOINT, map_location=DECODER_DEVICE, weights_only=False)
    decoder.load_state_dict(dec_ckpt)
    decoder.eval()
...
def train():
    ...
    val_loader = None
    if val_files:
        val_dataset = CompressedParnetDataset(val_files)
